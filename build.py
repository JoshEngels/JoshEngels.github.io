...
EXTERNAL_LINKS = {
    "github": "https://github.com/JoshEngels",
    "twitter": "https://twitter.com/JoshAEngels",
...
def generate_html(
    ...
    # Process hiking photos
    photo_groups = process_hiking_photos()

    generate_main_page(jinja_env, sorted_posts, context, out_dir)

    for post in posts:
        # Pass photo groups to hiking page
        extra_context = {}
        if post.meta.label == "hiking":
            extra_context["photo_groups"] = photo_groups
        generate_post_page(jinja_env, post, context, out_dir, posts, extra_context)


def generate_post_page(
    env: Environment, post: Post, context: dict, out_dir: str, posts: list[Post], extra_context: dict = None
):
...
def group_photos_by_time(photos_dir: str = "src/hiking_photos", gap_days: int = 5):
    """Group photos where consecutive photos are within gap_days of each other."""
    ...
    for photo in photo_files:
        ...
        timestamp = get_photo_timestamp(photo_path)
        photos_with_time.append((photo, timestamp))

    # Sort by timestamp
    photos_with_time.sort(key=lambda x: x[1])

    # Group by connected components (within gap_days)
    groups = []
    current_group = [photos_with_time[0]]

    for i in range(1, len(photos_with_time)):
        photo, timestamp = photos_with_time[i]
        prev_photo, prev_timestamp = photos_with_time[i-1]

        # Check if within gap_days of the previous photo
        if (timestamp - prev_timestamp).days <= gap_days:
...
def load_photo_group_names(names_file: str = "src/photo_groups.yaml"):
    """Load photo group names from YAML file."""
...
def process_hiking_photos(photos_dir: str = "src/hiking_photos",
                         out_dir: str = "out/hiking/photos"):
    """Process hiking photos and copy to output directory."""
    groups = group_photos_by_time(photos_dir)
    group_names = load_photo_group_names()
    print(group_names)
    ...
    group_data = []
    for group in groups:
        # Get earliest timestamp in group
        earliest_timestamp = min(ts for _, ts in group)
        timestamp_str = earliest_timestamp.strftime("%Y-%m-%d")  # Date only

        # Look up group name or use timestamp
        group_name = group_names.get(timestamp_str, timestamp_str)

        # Copy photos to output directory
        photo_paths = []
        for photo, _ in group:
            src_path = os.path.join(photos_dir, photo)
            dst_path = os.path.join(out_dir, photo)
            shutil.copy2(src_path, dst_path)
            photo_paths.append(f"photos/{photo}")
        ...
        group_data.append({
            ...
            "date": earliest_timestamp  # Keep full datetime for sorting
        })

    # Sort by date, most recent first
    group_data.sort(key=lambda x: x["date"], reverse=True)
